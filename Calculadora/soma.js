...
export async function sum(){
    ...
    console.log("Bem-vindo ao programa de soma de dois números!");

    console.log("O sistema de soma é simples, você irá digitar os números que você deseja somar, e o sistema vai te retornar o resultado da soma");
    ...
    if(moreNumbers === "no"){
        ...
        const result = numbers.reduce((acc,n) => acc + n,0); // reduce reduz varios valores de um array a um unico valor, ele percorre o array elemento pot elemento
...
async function moreSum(){
    const moreFunction = await select({
        message:"Deseja realizar outra soma?",
        options:[
            {value:"yes", label:"Sim"},
            {value:"no", label:"Não"}
        ]
    ...
    if(moreFunction === "yes"){
        setTimeout(sum, 1000) // para voltar a função sum e realizar outra soma
    } else if(moreFunction === "no"){
        console.log("Você escolheu não realizar outra soma, obrigado por usar o programa!");
