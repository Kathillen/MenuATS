...
export async function area(){
    ...
    console.log("Seja bem vindo ao sistema cálculo de area");

    const lado = Number(await text({
        message:"Insira a medida de um lado do quadrado:"
    }));

    parseFloat(lado)
    
    console.log(`O número digitado foi ${lado}`);

    const resultArea = lado * lado;

    console.log(`A Área do quadrado com as mededidas iguais a ${lado} é:  ${resultArea}`);

    const otherArea = await select({
